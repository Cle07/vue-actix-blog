...
// Structure for the Lua code request
...
#[post("/api/lua/run")]
pub async fn lua_run(
    data: web::Data<AppState>,
    lua_req: web::Json<LuaRequest>,
) -> Result<HttpResponse> {
    use mlua::prelude::*;
    use std::sync::{Arc, Mutex};
    use tokio::time::{Duration, timeout};

    // println!("Running Lua code: {}", lua_req.code);

    // Check for potentially unsafe code patterns
    if lua_req.code.contains("os.") {
        return Ok(HttpResponse::BadRequest().json(json!({
            "error": "Code contains potentially unsafe operations (os module access)",
        })));
    }
